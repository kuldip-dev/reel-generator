  ...
  return (
    <main className="min-h-screen bg-[#0a0a0f] text-white">
      <AppHeader
        ...
        subtitle="Rendered in your browser · 1080×1920 · MP4"
      />
        ...
        {images.length > 0 && (
          <section
            className={`bg-white/3 border rounded-2xl p-5 transition-all ${
              audioConfig ? "border-violet-500/30" : "border-white/8"
            }`}
          >
            <SectionLabel
              number={3}
        ...
        {images.length > 0 && (
          <div className="flex flex-wrap gap-3 text-xs">
            ...
            {audioConfig && (
              <Pill
                icon="🎵"
                label={`Audio · ${audioConfig.file.name.split(".").slice(0, -1).join(".").slice(0, 20)}`}
              />
            )}
          </div>
        )}
